...
namespace UnitTest101
{
	...
	public class BankAccountNUnitTest
	{
		...
		public void Deposit_VerifyMoqsProperties()
		{
			...
			account.Deposit(111);
			//Verificar si _logBook.Message fue invocado en Deposit() al menos una vez (line 18)
			logBookMoq.Verify(x => x.Message(It.IsAny<string>()), Times.AtLeastOnce());
			//Verificar si _logBook.Message fue invocado en Deposit() exactamente una vez  (line 18)
			logBookMoq.Verify(x => x.Message(It.IsAny<string>()), Times.Exactly(1));
			//Verificar si _logBook.Severity fue seteado en Deposit() fue invocado una vez (line 19)
			logBookMoq.VerifySet(x => x.LogSeverity = 1, Times.Once);
			//Verificar si _logBook.Severity Get en Deposit()fue llamado una vez(line 20)
